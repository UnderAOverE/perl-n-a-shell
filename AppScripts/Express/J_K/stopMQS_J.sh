...
qmgrs=''
/usr/mqm/bin/dspmq | grep "TTI.J" | grep -i running | sed 's/[()]/ /g' | while read line
do
	set -- $line
	qmgrs="$qmgrs $2"
done
...
while [ $running -eq 1 ]
do
	/usr/mqm/bin/dspmq | grep "TTI.J" | grep -ic ended | read qcnt
	/usr/mqm/bin/dspmq | grep "TTI.J" | wc -l | read lines
	if [ $qcnt -eq $lines ] ; then
		echo "all J QMgrs are down"
		running=0
	else
		echo "$qcnt of $lines of J QMgrs are down"
	fi
	sleep 3
done
...
sleep 5
ps -ef | grep "TTI.J" | grep -v grep | grep runmqlsr | awk '{print $2}' | xargs -i kill -9 {} > /dev/null 2>&1