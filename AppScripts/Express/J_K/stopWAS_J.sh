...
if [ "$1" = 'kill' ] ; then
	ps -ef | grep 'java.*WS' | grep 'WebSphereJ' | grep -v grep | awk '{print $2}' | while read pid
	do
		kill -9 $pid
	done
	sleep 30
...
APP1=/usr/WebSphereJ/Express/profiles/JNode01/bin
APP2=/usr/WebSphereJ/Express/profiles/JNode02/bin
DEP=/usr/WebSphereJ/Express/profiles/JDmgr/bin

case `hostname` in
	txwast11)		
		echo "stop tti_WLM_jserver1"
		$sh -c "$APP1/stopServer.sh tti_WLM_jserver1"
		echo "stop tti_WLM_jserver2"
		$sh -c "$APP2/stopServer.sh tti_WLM_jserver2"
		echo "stop Node1"
