...
######################################################################################################################
#
# name: startStandalone.sh
# author: Shane Reddy
# version: 1.0.0v
# dob: 01/28/2020
# explanation: Start script for EAP 7.2 Standalone servers.
...
function Echo {
    ...
    if [[ ${log_me} -eq 1 ]]; then
       if [[ ${log_level} -eq 0 ]]; then
          if [[ ${debug_messages} -eq 1 ]]; then
             echo "${date_recorded}|${host_name}|debug| ${Message}" >> ${log_directory}/startStandalone.log
          fi
       elif [[ ${log_level} -eq 1 ]]; then
          echo "${date_recorded}|${host_name}|INFO| ${Message}" >> ${log_directory}/startStandalone.log
       elif [[ ${log_level} -eq 2 ]]; then
          echo "${date_recorded}|${host_name}|WARN| ${Message}" >> ${log_directory}/startStandalone.log
       elif [[ ${log_level} -eq 3 ]]; then
          echo "${date_recorded}|${host_name}|ERROR| ${Message}" >> ${log_directory}/startStandalone.log
       elif [[ ${log_level} -eq 4 ]]; then
          echo "${date_recorded}|${host_name}|FATAL| ${Message}" >> ${log_directory}/startStandalone.log
       else
          echo "${date_recorded}|${host_name}|??| ${Message}" >> ${log_directory}/startStandalone.log
       fi
    fi
}
...
function check_if_app_is_ready {
    ...
    while true; do
        ppqa_return=$(python ${script_directory}/py/app_ready.py "PaymentProcessing QA" "http://${ppqastandalone_address}:${jvm_port}" "200")
        Echo ${got_debug} ${got_verbose} ${got_logger} 0 "ppqa_return=${ppqa_return}."
        if [[ ${ppqa_return} -eq 0 ]]; then
            Echo ${got_debug} ${got_verbose} ${got_logger} 1 "Application (PaymentProcessing QA) is up and ready now on ${host_name}."
            break;
        else
            Echo ${got_debug} ${got_verbose} ${got_logger} 1 "Application (PaymentProcessing QA) start in-progress on ${host_name}."
        fi
        ...
        Echo ${got_debug} ${got_verbose} ${got_logger} 1 "status check again in ${application_start_time} seconds..."
...
function start_server {
    ...
    if [[ $? -eq 0 ]]; then
        Echo ${got_debug} ${got_verbose} ${got_logger} 2 "Server (Standalone) @ ${host_name}:${management_port} is already running."
    else
        Echo ${got_debug} ${got_verbose} ${got_logger} 1 "Starting (Standalone) @ ${host_name}:${management_port}..."
        sleep 1
        sudo -H -E -u jboss bash -c 'nohup ${jboss_home}/bin/standalone.sh -c standalone.xml -b 0.0.0.0 1>${log_directory}/paymentQA_nohup.out 2>${log_directory}/paymentQA_nohup.err &'
        sleep 10
        ...
        if [[ $? -eq 0 ]]; then
            Echo ${got_debug} ${got_verbose} ${got_logger} 1 "Started server (Standalone) @ ${host_name}:${management_port}."
            check_if_app_is_ready
        else
            Echo ${got_debug} ${got_verbose} ${got_logger} 3 "Failed to start server (Standalone) @ ${host_name}:${management_port}!"
...
function print_help {
    echo ""
    echo "      ./startStandalone.sh.sh --server or --version or --help or --info"
    echo "          shortcut convention: ./startStandalone.sh.sh [-s/ -v/ -h/ -i]"
    echo ""
    echo "      --server|-s --> performs start operation on Standalone server/ JVM."
    echo ""
...
######################################################################################################################
clear_screen
echo "" >> ${log_directory}/startStandalone.log; echo "---" >> ${log_directory}/startStandalone.log
