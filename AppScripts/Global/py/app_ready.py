...
#                (01/28/2020 1.2.1v) fixed 'try' always returns '200' if valid URL, ignores user input code.
#
...
def is_app_ready(appname, url, http_code, timeout=2):
    ...
    try:
        ...
        app_http_code=urllib2.urlopen(url, timeout=timeout, context=context).getcode()
        if int(http_code) == app_http_code: return 0
        return 1
...
if __name__== "__main__":
    if len(sys.argv) == 4:
        ...
        print("ERROR| provide Application name, URL & expected HTTP Code.")
