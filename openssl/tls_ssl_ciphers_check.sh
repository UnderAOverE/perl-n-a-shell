			...
			if [[ ${protocol_tls12} -eq 1 ]]; then
				...
				for Cipher in $(openssl ciphers | awk -F ":" '{ for (cph=1;cph<NF+1;cph++) if (length($cph) != 0) print $cph }'); do
					...
					if [[ $? -eq 0 ]]; then
						...
						Echo "  ${connection_host}[:${connection_port}] using [TLSv1.2] with ${Cipher} --> Passed."
						tls12_check=1
					else
						x=0;
						#echo "  ${connection_host}[:${connection_port}] using [TLSv1.2] with ${Cipher} --> ${BLINK}${RED}Failed${NORMAL}."
						#Echo "  ${connection_host}[:${connection_port}] using [TLSv1.2] with ${Cipher} --> Failed."
					fi
				done
				[[ ${tls12_check} -eq 0 ]] && { echo "  ${connection_host}[:${connection_port}] using [TLSv1.2] --> ${BLINK}${YELLOW}Not Allowed${NORMAL}." ; Echo "  ${connection_host}[:${connection_port}] using [TLSv1.2] --> Not Allowed." ; }
			fi
			if [[ ${protocol_tls11} -eq 1 ]]; then
				...
				for Cipher in $(openssl ciphers | awk -F ":" '{ for (cph=1;cph<NF+1;cph++) if (length($cph) != 0) print $cph }'); do
					...
					if [[ $? -eq 0 ]]; then
						...
						Echo "  ${connection_host}[:${connection_port}] using [TLSv1.1] with ${Cipher} --> Passed."
						tls11_check=1
					else
						x=0;
						#echo "  ${connection_host}[:${connection_port}] using [TLSv1.1] with ${Cipher} --> ${BLINK}${RED}Failed${NORMAL}."
						#Echo "  ${connection_host}[:${connection_port}] using [TLSv1.1] with ${Cipher} --> Failed."
					fi
				done
				[[ ${tls11_check} -eq 0 ]] && { echo "  ${connection_host}[:${connection_port}] using [TLSv1.1] --> ${BLINK}${YELLOW}Not Allowed${NORMAL}." ; Echo "  ${connection_host}[:${connection_port}] using [TLSv1.1] --> Not Allowed." ; }
			fi
			if [[ ${protocol_tls10} -eq 1 ]]; then
				...
				for Cipher in $(openssl ciphers | awk -F ":" '{ for (cph=1;cph<NF+1;cph++) if (length($cph) != 0) print $cph }'); do
					...
					if [[ $? -eq 0 ]]; then
						...
						Echo "  ${connection_host}[:${connection_port}] using [TLSv1.0] with ${Cipher} --> Passed."
						tls10_check=1
					else
						x=0;
						#echo "  ${connection_host}[:${connection_port}] using [TLSv1.0] with ${Cipher} --> ${BLINK}${RED}Failed${NORMAL}."
						#Echo "  ${connection_host}[:${connection_port}] using [TLSv1.0] with ${Cipher} --> Failed."
					fi
				done
				[[ ${tls10_check} -eq 0 ]] && { echo "  ${connection_host}[:${connection_port}] using [TLSv1.0] --> ${BLINK}${YELLOW}Not Allowed${NORMAL}." ; Echo "  ${connection_host}[:${connection_port}] using [TLSv1.0] --> Not Allowed." ; }
			fi
			if [[ ${protocol_ssl3} -eq 1 ]]; then
				...
				for Cipher in $(openssl ciphers | awk -F ":" '{ for (cph=1;cph<NF+1;cph++) if (length($cph) != 0) print $cph }'); do
					...
					if [[ $? -eq 0 ]]; then
						...
						Echo "  ${connection_host}[:${connection_port}] using [SSLv3] with ${Cipher} --> Passed."
						ssl3_check=1
					else
						x=0;
						#echo "  ${connection_host}[:${connection_port}] using [SSLv3] with ${Cipher} --> ${BLINK}${RED}Failed${NORMAL}."
						#Echo "  ${connection_host}[:${connection_port}] using [SSLv3] with ${Cipher} --> Failed."
					fi
				done
				[[ ${ssl3_check} -eq 0 ]] && { echo "  ${connection_host}[:${connection_port}] using [SSLv3] --> ${BLINK}${YELLOW}Not Allowed${NORMAL}." ; Echo "  ${connection_host}[:${connection_port}] using [SSLv3] --> Not Allowed." ; }
			fi
			if [[ ${protocol_ssl2} -eq 1 ]]; then
				...
				for Cipher in $(openssl ciphers | awk -F ":" '{ for (cph=1;cph<NF+1;cph++) if (length($cph) != 0) print $cph }'); do
					...
					if [[ $? -eq 0 ]]; then
						...
						Echo "  ${connection_host}[:${connection_port}] using [SSLv2] with ${Cipher} --> Passed."
						ssl2_check=1
					else
						x=0;
						#echo "  ${connection_host}[:${connection_port}] using [SSLv2] with ${Cipher} --> ${BLINK}${RED}Failed${NORMAL}."
						#Echo "  ${connection_host}[:${connection_port}] using [SSLv2] with ${Cipher} --> Failed."
					fi
				done
				[[ ${ssl2_check} -eq 0 ]] && { echo "  ${connection_host}[:${connection_port}] using [SSLv2] --> ${BLINK}${YELLOW}Not Allowed${NORMAL}." ; Echo "  ${connection_host}[:${connection_port}] using [SSLv2] --> Not Allowed." ; }
			fi
		else
			echo "  ${connection_host}:${connection_port} --> ${BLINK}${RED}Connection Refused${NORMAL}."
			Echo "  ${connection_host}:${connection_port} --> Connection Refused."
		fi
	else
